    ...
    def __call__(self, imgs, ball_position_xys, visibilities):
        if random.random() <= self.p:
            for t in self.transforms:
                result = t(imgs, ball_position_xys, visibilities)
...
class Resize(object):
    ...
    def __call__(self, imgs, ball_position_xys, visibilities):
        ...
        transformed_imgs = []
        transformed_ball_pos_list = []
        for img, ball_pos in zip(imgs, ball_position_xys):
            transformed_img = cv2.resize(img, (new_h, new_w), interpolation=self.interpolation)
            transformed_ball_pos = np.array([ball_pos[0] / w_ratio, ball_pos[1] / h_ratio])
            transformed_imgs.append(transformed_img)
            transformed_ball_pos_list.append(transformed_ball_pos)
        
        return transformed_imgs, transformed_ball_pos_list, visibilities
...
class Random_Crop(object):
    ...
    def __call__(self, imgs, ball_position_xys, visibilities):
        ...
        transformed_ball_pos_list = ball_position_xys.copy()
        ...
        if random.random() <= self.p:
            ...
            transformed_imgs = []
            transformed_ball_pos_list = []
            for i, (img, ball_pos) in enumerate(zip(imgs, ball_position_xys)):
                ...
                transformed_ball_pos = np.array([(ball_pos[0] - min_x) * w_ratio,
                                        (ball_pos[1] - min_y) * h_ratio])
                transformed_ball_pos_list.append(transformed_ball_pos)
    ...
    def __call__(self, imgs, ball_position_xys, visibilities):
        ...
        if random.random() <= self.p:
            ...
            for img, ball_pos in zip(imgs, ball_position_xys):
                ...
                transformed_ball_pos = rotate_matrix.dot(np.array([ball_pos[0], ball_pos[1], 1.]).T)
                transformed_ball_pos_list.append(transformed_ball_pos)
...
class Random_HFlip(object):
    ...
    def __call__(self, imgs, ball_position_xys, visibilities):
        transformed_imgs = imgs.copy()
        transformed_ball_position_list = ball_position_xys.copy()
        if random.random() <= self.p:
            h, w, c = imgs[0].shape
            transformed_imgs = []
            transformed_ball_position_list = []
            for img, ball_pos in zip(imgs, ball_position_xys):
                # Horizontal flip a sequence of imgs
                transformed_img = cv2.flip(img, 1)
                transformed_imgs.append(transformed_img)
                # Adjust ball position: Same y, new x = w - x
                transformed_ball_pos = (w - ball_pos[0], ball_pos[1])
                transformed_ball_position_list.append(transformed_ball_pos)
        return transformed_imgs, transformed_ball_position_list, visibilities
    ...
    def __call__(self, imgs, ball_position_xys, visibilities):
        ...
        if random.random() <= self.p:
            ...
            for img, ball_pos in zip(imgs, ball_position_xys):
                ...
                transformed_imgs.append(transformed_img)
                transformed_ball_pos = (ball_pos[0], h-ball_pos[1])
                transformed_ball_position_list.append(transformed_ball_pos)
